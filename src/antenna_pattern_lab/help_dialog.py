...
HELP = {
    "CZE": {
        ...
        "sections": [
            ...
            (
                ...
                <p>NEC úhel theta se měří od zenitu. Workbench jej převádí na
                praktickou elevaci nad horizontem jako 90° − theta. 3D diagram
                proto zobrazuje rovinu horizontu, azimuty a označený paprsek
                maxima. Panel praktické interpretace uvádí podíly vyzařování
                podle elevace, rádiový horizont a geometrickou vzdálenost
                jednoho skoku pro předpokládanou E vrstvu 110 km a F2 300 km.</p>
                <p>Tyto vzdálenosti nejsou předpověď šíření: neobsahují
                foF2/MUF, absorpci, aktuální ionosféru, terén ani link budget.
                Far-field NEC diagram také sám nepočítá dosah přízemní vlny.</p>
                ...
                povinně hodnotí na pozdějších nepoužitých datech. Verze 0.43.0
    ...
    "ENG": {
        ...
        "sections": [
            ...
            ("nec_workbench", "NEC2 antenna modeling", "<h2>Antenna Modeling · NEC2 Workbench</h2><p>Open <b>Tools → Antenna modeling (NEC2)</b> to create a wire model from Dipole, Inverted-V, Vertical, Loop, or Yagi templates. Tables edit wires, the feed point, series RLC loads, ground, and frequency sweep; the 3D preview and validation run before calculation.</p><p>Optional OpenNEC runs only as a standalone process. A normal run stores its engine version, exact parameters, UTC, input/output SHA-256, impedance, SWR, currents, and 2D/3D radiation as an immutable theoretical baseline. Models and saved results remain usable without the solver.</p><p>NEC theta is measured from zenith. The workbench converts it to practical elevation above the horizon as 90° − theta. The 3D view draws the horizon, azimuth references, and marked peak ray. Practical interpretation reports elevation power shares, radio horizon, and geometric one-hop distances for assumed 110 km E and 300 km F2 virtual heights.</p><p>Those distances are not propagation predictions: foF2/MUF, absorption, current ionospheric support, terrain, and link budget are excluded. The NEC far-field pattern also does not calculate ground-wave range.</p><p>Assisted height/ground candidates stay separate from the independent baseline. Orientation is selected on training time blocks and must report performance on later unused data. Version 0.43.0 supports the documented NEC2 wire subset—not patches, buildings, real coax, full terrain, or NEC4.</p>"),
